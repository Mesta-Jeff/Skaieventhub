                        ...
                        <div class="col-xxl-6 ms-auto">
                            <div>
                                <select id="filter2" class="select2 form-control">
                                    <option value="" selected>Filter by District...</option>
                                </select>
                            </div>
                        </div>
                ...
                <div class="modal-body">
                    ...
                    <div class="row g-9">
                        ...
                        <div class="col-md-12 mb-2">
                            <label for="status" class="form-text">Provide your prefered Name</label>
                            <input type="text" class="form-control" placeholder="eg. Suhum" id="town" onselectstart="return false" onpaste="return false;" oncopy="return false" oncut="return false" ondrag="return false" ondrop="return false" autocomplete="off" required/>
                        </div>
                        <label for="status" class="text-info mb-2" id="tags">Do you want to modify the district..[
                            <a id="btnYes" class="text-success bold" href="javascript:void(0)" data-decision="yes">Yes I want to</a>,
                            <a id="btnNo" class="text-danger bold" href="javascript:void(0)" data-decision="no">No I don't</a> ]
                        </label>
                        <section class="col-md-12" id="district-unit">
                            <div class="col-md-12 mb-2">
                                <select id="region" class="select2 form-control mb-2" data-toggle="select2">
                                </select>
                            </div>
                            <div class="col-md-12 mb-2">
                                <select id="district" class="select2 form-control mb-2" data-toggle="select2">
                                </select>
                            </div>
                        </section>
...
<script>
    $(document).ready(function ()
    {
        ...
        // Making decision on what to udate
        $('a[data-decision]').click(function() {
        ...
        $('#region').change(function() {
            ...
            if (selected_id) {
                $.ajax({
                    ...
                    type: 'GET',
                    data: { region_id: selected_id },
                    dataType: 'json',
                    success: function (data) {
                        $('#district').empty();
                        $('#district').append($('<option>', {
                            value: '',
                            text: 'Town is in what District...?',
                            selected: 'selected',
                            disabled: 'disabled'
                        }));
        ...
        $('#btnNew').click(function() {
            $('#my-form')[0].reset();
            $('#state-view, #edit-data, #tags').hide();
            $('#save-data').show();
            $('#district').val('').change();
            $('#region').val('').change();
        ...
        $('#example').on('click', '.edit-btn', function() {
            ...
            $('#oldDistrict').val(district_id);
            $('#town').val(town);
            $('#gottenid').val(id);
            $('#stat').val(status);
            $('#save-data, #district-unit').hide();
            $('#edit-data, #state-view, #tags').show();
            $('#my-modal').modal('show');
        });
                ...
                },
                {
                    ...
                    render: function(data, type, row) {
                        return '<div class="btn-group">' +
                            ...
                            '<a class="dropdown-item edit-btn" href="javascript: void(0);" data-id="' + data.id + '" data-town="' + data.name + '" data-district="' + data.district_id + '" data-status="' + data.status + '"><i class="mdi mdi-pen-plus mx-1"></i>Modify Record </a>' +
        ...
        $('#save-data').on('click', function() {
            ...
            if (!validateForm(towns, district_id)) return;
            ...
            $.ajax({
                ...
                data: {_token: '{{ csrf_token() }}',name: towns, district_id: district_id},
                success: function(response) {
        ...
        $('#edit-data').on('click', function() {
            ...
            {
                district_id = $('#oldDistrict').val();
            }

            // Check if fields are not empty
            if (!validateForm(towns, stat,district_id, gottenid)) return;
            ...
            $.ajax({
                type: "POST",
                url: '{{ route("settings.towns.update") }}',
                data: {
                    _token: '{{ csrf_token() }}',
                    name: towns, status: stat,town_id: gottenid, district_id:district_id
                },
                success: function(response) {
                    console.log('Success Response:', response);
        ...
        $('#example').on('click', '.delete-btn', function(e) {
            ...
            Swal.fire({
                ...
                if (result.isConfirmed) {
                    $.ajax({
                        type: "POST",
                        url: '{{ route("settings.towns.destroy") }}',
                        data: {_token: '{{ csrf_token() }}',town_id: ids},
                        success: function(response) {
                            console.log('Success Response:', response);
                            if (response.status="success") {
                                Swal.fire({ icon: 'success', title: 'Success',text: response.message,})
