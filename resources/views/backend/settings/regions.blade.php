        ...
        <div class="col-12">
            <div class="card">
                <div class="card-body">
                    <div class="row mb-2">
                        <div class="col-sm-5">
                            <button class="btn btn-danger" type="button" id="bulk-remove" style="display: none" > Remove Selection</button>
                        </div>
                    </div>
                ...
                <div class="modal-body">
                    ...
                    <div class="row g-9">
                        <div class="col-md-12 mb-2">
                            ...
                            <input type="text" class="form-control" placeholder="eg. Eastern Region" id="region" onselectstart="return false" onpaste="return false;" oncopy="return false" oncut="return false" ondrag="return false" ondrop="return false" autocomplete="off" required/>
                        </div>
...
<script>
    $(document).ready(function ()
    {
        ...
        dataTable = $('#example').DataTable({
            ajax: {
                url: '{{ route("settings.regions.show") }}',
                dataSrc: 'regions'
            },
            columns: [
                {
                ...
                },
                {
                    ...
                    render: function(data, type, row) {
                        return '<div class="btn-group">' +
                            ...
                            '<a class="dropdown-item edit-btn" href="javascript: void(0);" data-id="' + data.id + '" data-region="' + data.name + '" data-status="' + data.status + '"><i class="mdi mdi-pen-plus mx-1"></i>Modify Record </a>' +
        ...
        $('#save-data').on('click', function() {
            ...
            if (!validateForm(regions)) return;
            ...
            $.ajax({
                type: 'POST',
                url: '{{ route("settings.regions.create") }}',
                data: {_token: '{{ csrf_token() }}',name: regions},
                success: function(response) {
        ...
        $('#edit-data').on('click', function() {
            ...
            // Check if fields are not empty
            if (!validateForm(regions, stat, gottenid)) return;
            ...
            $.ajax({
                type: "POST",
                url: '{{ route("settings.regions.update") }}',
                data: {
                    _token: '{{ csrf_token() }}',
                    name: regions, status: stat,region_id: gottenid
                },
                success: function(response) {
                    console.log('Success Response:', response);
        ...
        $('#example').on('click', '.delete-btn', function(e) {
            ...
            Swal.fire({
                ...
                if (result.isConfirmed) {
                    $.ajax({
                        type: "POST",
                        url: '{{ route("settings.regions.destroy") }}',
                        data: {_token: '{{ csrf_token() }}',region_id: ids},
                        success: function(response) {
                            console.log('Success Response:', response);
                            if (response.status="success") {
                                Swal.fire({ icon: 'success', title: 'Success',text: response.message,})
        ...
        $("#region").val("");   //resetting the regions
