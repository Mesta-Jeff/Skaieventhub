...
@section('title', 'List of Identities')
                ...
                <div class="modal-body">
                    ...
                    <div class="row g-9">
                        <div class="col-md-12 mb-2">
                            <label for="status" class="form-text">Provide your prefered identity type</label>
                            <input type="text" class="form-control" placeholder="eg. Voters ID" id="identitytype" onselectstart="return false" onpaste="return false;" oncopy="return false" oncut="return false" ondrag="return false" ondrop="return false" autocomplete="off" required/>
                        </div>
...
<script>
    $(document).ready(function ()
    {
        ...
        $('#example').on('click', '.edit-btn', function() {
            var id = $(this).data('id');
            var identitytype = $(this).data('identitytype');
            var status = $(this).data('status');

            $('#modal-title').text('About to modify this record');
            $('#identitytype').val(identitytype).change();
            $('#gottenid').val(id);
            $('#stat').val(status);
        ...
        dataTable = $('#example').DataTable({
            ajax: {
                url: '{{ route("settings.identitytypes.show") }}',
                dataSrc: 'identitytypes'
            },
            columns: [
                {
                    data: null,
                ...
                },
                {
                    ...
                    render: function(data, type, row) {
                        return '<div class="btn-group">' +
                            ...
                            '<a class="dropdown-item edit-btn" href="javascript: void(0);" data-id="' + data.id + '" data-identitytype="' + data.name + '" data-status="' + data.status + '"><i class="mdi mdi-pen-plus mx-1"></i>Modify Record </a>' +
        ...
        $('#filter2').on('change', function () {
            ...
            dataTable.column(3).search(catFilter).draw();
        });
        ...
        $('#save-data').on('click', function() {
            var identitytypes = $('#identitytype').val();

            if (!validateForm(identitytypes)) return;
            ...
            $.ajax({
                type: 'POST',
                url: '{{ route("settings.identitytypes.create") }}',
                data: {_token: '{{ csrf_token() }}',name: identitytypes},
                success: function(response) {
        ...
        $('#edit-data').on('click', function() {
            ...
            if (!validateForm(identitytypes, stat, gottenid)) return;
            ...
            $.ajax({
                type: "POST",
                url: '{{ route("settings.identitytypes.update") }}',
                data: {
                    _token: '{{ csrf_token() }}',
                    name: identitytypes, status: stat, identitytype_id: gottenid
                },
                success: function(response) {
                    console.log('Success Response:', response);
                    if (response.status="success") {
        ...
        $('#example').on('click', '.delete-btn', function(e) {
            ...
            Swal.fire({
                ...
                if (result.isConfirmed) {
                    $.ajax({
                        type: "POST",
                        url: '{{ route("settings.identitytypes.destroy") }}',
                        data: {_token: '{{ csrf_token() }}',identitytype_id: ids},
                        success: function(response) {
                            console.log('Success Response:', response);
                            if (response.status="success") {
                                Swal.fire({ icon: 'success', title: 'Success',text: response.message,})
        ...
        $("#identitytype").val("");   //resetting the identitytypes
